...
export const onRequestPost = async ({
  ...
  try {
    ...
    if (!secret) {
      return new Response(
        JSON.stringify({ error: "Session not configured" }),
        {
          status: 500,
          headers: { "Content-Type": "application/json" }
        }
    ...
    if (!body || !Array.isArray(body.games)) {
      return new Response(
        JSON.stringify({ error: "Invalid games payload" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" }
        }
    ...
    for (const g of body.games) {
      if (!g || typeof g.id !== "string" || typeof g.title !== "string") {
        return new Response(
          JSON.stringify({ error: "Invalid game entry in payload" }),
          {
            status: 400,
            headers: { "Content-Type": "application/json" }
          }
