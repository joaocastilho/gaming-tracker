<script lang="ts">
	...
	// Filter and display tiered games
	let displayedGames = $derived.by(() => {
		if (isLoadingGames || allGames.length === 0) {
			return [];
		}

		// Only show games with a tier
		let filteredGames = allGames.filter((game) => game.tier);
		...
		// Apply status filter (Tier list usually shows all statuses, but if filter is applied, respect it)
		if (currentFilterState.statuses.length > 0) {
			filteredGames = filteredGames.filter((game) =>
				currentFilterState.statuses.includes(game.status)
			);
		}
		...
		// Sort alphabetically by default for tier list (within tiers)
