...
class DataPersistenceTester {
  ...
  setupMocks() {
    global.localStorage = this.localStorage as any;
  ...
  async testFilterURLPersistence(): Promise<boolean> {
    try {
      ...
      params.set('ratingPresentation', `${TEST_FILTER_STATE.ratingPresentation[0]}-${TEST_FILTER_STATE.ratingPresentation[1]}`);
